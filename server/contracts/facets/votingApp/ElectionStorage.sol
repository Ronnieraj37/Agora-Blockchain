...
contract ElectionStorage {
    ...
    function removeElection(
        address[] memory _elections,
        address _election
    ) public pure returns (address[] memory) {}

    function deleteElection(address _election) external {
        require(
            getElectionOwner(_election) == msg.sender,
            "Only Owner Can Delete Election"
        );
        Election election = Election(_election);
        require(
            election.getStatus() == Election.Status.pending,
            "Cannot delete election after election has started"
        );
    ...
    function addOrganizerToInviteBasedElection(
        ...
        require(
            isOrganizerAddedToElection[_election][_addOrganizer] == false,
            "Organizer already added to this election"
        );
