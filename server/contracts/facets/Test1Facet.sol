...
contract Test1Facet {
    ...
    function setReqResult() external {
        GetResultCalculator(LibDiamond.addressStorage().diamond)
            .getNewResultCalculator(2);
    }
    ...
    function test1facetelection()
        external
        pure
        returns (LibDiamond.ElectionStorage memory)
    {
        return LibDiamond.electionStorage();
    }
    function test1facetAddress()
        external
        pure
        returns (LibDiamond.AddressStorage memory)
    {
