  ...
  const fetchElectionDetails = async () => {
    try {
      ...
      if (ethereum) {
        ...
        //set election status
        updateStatus(
          Number(electionDetail.startDate._hex),
          Number(electionDetail.endDate._hex)
        );

        //set ballot type and algorithm type
        const ballotType = Number(
          (await electionContract.getBallotType())._hex
        );
        ...
        else if (ballotType == 7) setElectionAlgorithm("Kemeny Young");
        else if (ballotType == 8) setElectionAlgorithm("Ranking");
  ...
  const getElectionOrganizer = async () => {
    try {
      ...
      if (ethereum) {
        ...
        //fetched election details
        const electionOrg = await organizerContract.getElectionOwner(
          electionAddress
        );
  ...
  const getWinnerDetails = async () => {
    ...
    try {
      ...
      if (ethereum) {
        ...
        let _winnerDetails = [];
        id = toast.loading("Processing Your Vote Request", {
          theme: "dark",
          position: "top-center",
        });
    ...
    try {
      ...
      if (Date.now() >= edate) {
        ...
        if (ethereum) {
          const provider = new ethers.providers.Web3Provider(ethereum);
          const signer = provider.getSigner();
          id = toast.loading("Processing Your Request", {
            theme: "dark",
            position: "top-center",
          });
          const electionContract = new ethers.Contract(
    ...
    <div style={{ backgroundColor: "#f7f7f7", minHeight: "100%" }}>
      ...
      <div style={{ padding: "30px" }}>
        <div style={{ width: "100%" }}>
          <div style={{ float: "left" }}>
            <div style={{ display: "flex" }}>
              <h5 style={{ marginBottom: "0px", width: "max-content" }}>
            ...
            <font size="2" className="text-muted" style={{ marginTop: "0px" }}>
              {electionAddress}
            </font>
          </div>
          ...
          <div style={{ float: "right", display: "flex" }}>
            <MyTimer
              ...
              sdate={electionDetails.startDate}
              edate={electionDetails.endDate}
            />
            {isAdmin === publicAddress && status == STATUS.PENDING && (
              <div style={{ display: "flex" }}>
                {isOpenBasedElection == true && (
                  <DeleteModal
                    orgnizerAddress={organizerAddress}
        ...
        <div className="cardContainer row">
          <CardItem
            headerValue={electionAlgorithm}
            descriptor="Algorithm"
            imgUrl="/assets/totalElections.png"
          />

          <CardItem
            headerValue={new Date(
              electionDetails?.startDate * 1000
            ).toLocaleString()}
          ...
          <CardItem
            headerValue={voterscount}
            descriptor="Total voters"
        ...
        <div className="layoutBody row">
          <div className="lhsLayout" style={{ overflowY: "scroll" }}>
            {status == STATUS.CLOSED ? (
              ...
              <span
                onClick={() => {
                  console.log("Result only calculated after election ends");
                }}
                className="voteButton voteButtonDisabled"
                style={{ float: "right", marginTop: "10px", width: "100px" }}
              >
                Get Result
              </span>
                    ...
                    (candidate) =>
                      candidate?.name !== "" && (
                        <Candidate
                          ...
                          imageUrl={
                            AVATARS[candidate?.id % AVATARS?.length] ||
                            "/assets/avatar.png"
                          }
                        />
                      )
            ...
            <div className="lhsBody" style={{ textAlign: "justify" }}>
              ...
              <font size="2" className="text-muted">
                {ballotType === 1
                  ...
                  : ballotType == 2
                  ? `The Oklahoma primary electoral system is
                  ...
                  ? `The Borda count method is a point-based election system in which voters number their 
								preferred choices in order. The Borda count method does not rely on the majority criterion
								 or Condorcet criterion. The majority criterion states if one choice gets the majority of
								  ...
								  winner.`
                  : ballotType == 5
                  ? `The Schulze method is a Condorcet method, which means that if there 
								  is a candidate who is preferred by a majority over every other candidate in pairwise comparisons,
								   ...
								   Schulze method gives an ordering of candidates.`
                  : ballotType == 6
